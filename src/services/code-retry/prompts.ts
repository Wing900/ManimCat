...
import { LATEX_KNOWLEDGE_SUPPLEMENT, SYSTEM_PROMPT_BASE } from '../../prompts'
...
- **结构规范**：核心类名固定为 \`MainScene\`（若是 3D 场景则继承 \`ThreeDScene\`）。
- **导入规范**：必须使用 \`from manim import *\`
...
- **配置字典**：严禁直接在 \`Axes\` 初始化中传入视觉参数，必须放在 \`axis_config\`。
